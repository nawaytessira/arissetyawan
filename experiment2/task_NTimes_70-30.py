...
dataname= 'play_data'
neuron= None
dataset= np.genfromtxt( DATA_PATH+ dataname + ".csv", delimiter=',', skip_header=0)
...
if len(sys.argv)!= 2:
    ...
    for trial in range(REPEAT):
        mydata= data(dataset=dataset,percTrain=percTrain, percTest=percTest, percVal=0.0, 
                    normType="maxmin", shuf=True, posOut="last", outBin=True)
        mydata.save("datasets/"+dataname+"_"+ str(trial) + ".csv")
        mydata.load()

        ###########################################################################
        print(line2())
        print (trial, "Start training ELM-RBM ...")
        ...
        elmNet1.saveELM("log/"+ dataname + "/" + str(neuron)  + "rbm_" + str(trial))
        del(elmNet1)

        ###########################################################################
        print(line())
        print (trial, "Start training ELM-RND...")  
        init2 = time.time()
        ...
        elmNet2.saveELM("log/"+ dataname  + "/" + str(neuron)  + "rnd_" + str(trial))
        del(elmNet2)
        ...
        elmNet3.saveELM("log/"+ dataname + "/" + str(neuron) + "ro_" + str(trial))
        del(elmNet3)
    ...
    print ("\nRBM-ELM::")
    log("log/" + dataname + "/"+str(neuron)+ "elmrbm-acc", acc1)
    log("log/" + dataname + "/"+str(neuron)+ "elmrbm-tim", tim1)
    log("log/" + dataname + "/"+str(neuron)+ "elmrbm-norm", normRBMELM)
    log("log/" + dataname + "/"+str(neuron)+ "elmrbm-std", str(acc1.std()))
    text += "\nRBM-ELM:"
    ...
    log("log/" + dataname + "/"+str(neuron)+ "elmrnd-acc", acc2)
    log("log/" + dataname + "/"+str(neuron)+ "elmrnd-tim", tim2)
    log("log/" + dataname + "/"+str(neuron)+ "elmrnd-norm", normELM)
    log("log/" + dataname + "/"+str(neuron)+ "elmrnd-std", str(acc2.std()))
    text += "\nELM-RND:"
    ...
    log("log/" + dataname + "/"+str(neuron)+ "elmro-acc", acc3)
    log("log/" + dataname + "/"+str(neuron)+ "elmro-tim", tim3)
    log("log/" + dataname + "/"+str(neuron)+ "elmro-norm", normELM)
    log("log/" + dataname + "/"+str(neuron)+ "elmro-std", str(acc3.std()))
    text += "\nELM-RO:"
    ...
    print ("\nALL RESULT:")
    data = [acc1, acc2, acc3]
    log("log/" + dataname + "/"+str(neuron)+ "acc-all", data)
    log("log/" + dataname + "/"+str(neuron)+ "result", text)
