@mfunction("TestingTime, TestingAccuracy")
def elm_predict(TestingData_File=None):
    ...
    if Elm_Type != REGRESSION:
        ...
        for i in mslice[1:NumberofTestingData]:
            for j in mslice[1:size(label, 2)]:
    ...
    TestingTime = end_time_test - start_time_test; print TestingTime#   Calculate CPU time (seconds) spent by ELM predicting the whole testing data

    if Elm_Type == REGRESSION:
        TestingAccuracy = sqrt(mse(TV.T - TY)); print TestingAccuracy    #   Calculate testing accuracy (RMSE) for regression case
        output = TY
    end
    ...
    if Elm_Type == CLASSIFIER:
        ...
        TestingAccuracy = 1 - MissClassificationRate_Testing / NumberofTestingData; print TestingAccuracy
    end

    save(mstring('elm_output'), mstring('output'))