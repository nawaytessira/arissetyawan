...
mydataTrain = np.genfromtxt(DATA_PATH+ 'iris.csv', delimiter=',')
...
mydata = data (dataset=mydataTrain, posOut='last', percTrain=0.7, percVal=0.1, percTest=0.2, normType='maxmin')
# p(mydata)
# p(mydata.trainIn)
# p(mydata.trainOut)
# p(mydata.testIn)
...
for j in range(it):
    ...
    print ('ITERATION ', i)
    print(line2())
    print( 'Starting training ELM-RBM ')  
    
    init1 = time.time() # getting the start time
    ...
    elmNet1 = ELM (hidNeurons, mydata.trainIn, mydata.trainOut, W)
    elmNet1.train(aval=True)    
    end1 = time.time() # getting the end time     
    res, a = elmNet1.getResult (mydata.testIn,mydata.testOut,True)
    
    nor,_ = elmNet1.getNorm()
    normRBMELM.append(nor)  
    acc1.append(a)
    tim1.append(end1-init1)    

    ###########################################################################

    print ('Starting training ELM-RANDOM ')
    init2 = time.time()
    elmNet2 = ELM (hidNeurons, mydata.trainIn, mydata.trainOut, weight='uniform')
    elmNet2.train(aval=True)    
    end2 = time.time() # getting the end time     
    res, a = elmNet2.getResult (mydata.testIn,mydata.testOut,True)   
    ...
    ###########################################################################
    print ('Starting training ELM-RO ')
    init3 = time.time()
    elmNet3 = ELM (hidNeurons, mydata.trainIn, mydata.trainOut, weight='RO')
    elmNet3.train(aval=True)
    end3 = time.time()    
    res, a = elmNet3.getResult (mydata.testIn,mydata.testOut,True) 
    ...
    ###########################################################################       
    settings= line2() + "ELM settings-hidNeurons:" + str(hidNeurons)
    settings= br() + "RBM settings:" + str(rbmNet) + line2()

    text= line() + "Weight RBM: " + br() + str(elmNet1.W)
    text+= line() + "Weight RND: " + br() + str(elmNet2.W)
    text+= line() + "Weight RO: " + br() + str(elmNet3.W)
    text+= line2()
    log_iter("data", i, settings, text)
