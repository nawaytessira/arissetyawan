...
neuron= None
if len(sys.argv)!= 4:
    print("Please pass argument for datasetname fold neuron eg. iris 3 100")
else:
    dataset= sys.argv[1]
    K= int(sys.argv[2])
    neuron= int(sys.argv[3])
    print("Dataset, K, Neurons: ", dataset, K, neuron)
    ...
    objFold.KFold() #if not already splitted by KFold_creator.py 3

    for fold in range(K-1):
        train= objFold.loadTrain(fold_number)
        test= objFold.loadTest(fold_number)
        # print(test)
        mydata= data(train=train, test=test, val=None, normType="maxmin", shuf=True, posOut="first", outBin=False)
