...
dname= "iris"
df= pd.read_csv(DATA_PATH+ dname +".csv", header=0, dtype=None)
from sklearn.preprocessing import LabelEncoder
obj_df = df.select_dtypes(include=["object"]).copy()
obj_df["class"] = obj_df["class"].astype("category")
obj_df.dtypes
obj_df["coded_class"] = obj_df["class"].cat.codes
obj_df.head()

df= df.assign(coded_class=obj_df.coded_class)
colsToKeep= ["sepal_length", "sepal_width", "petal_length", "petal_width","coded_class"]
mydataset= df[colsToKeep].reset_index().values
print(mydataset)
...
for t in range(trials):
    ...
    elmNet1.saveELM("log/iris-rbm_" + str(trial) + "-" + str(percTrain) + "-" + str(percTest))
    del(elmNet1)
    ...
    elmNet2.saveELM("log/iris-rnd_" + str(trial) + "-" + str(percTrain) + "-" + str(percTest))
    del(elmNet2)
    ...
    elmNet3.saveELM("log/iris-ro_" + str(trial) + "-" + str(percTrain) + "-" + str(percTest))
    del(elmNet3)
